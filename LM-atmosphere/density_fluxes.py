...
def density_fluxes(my_data, u_adv, v_adv, rp, dt, scalar_name):
    """
    Construct the fluxes through the interfaces for the a conservatively
    advectived quantity (like density)
    ...
    In this case, there is no Riemann problem we need to solve -- we 
    just simply do upwinding based on the input advective velocities.  
    So there is only one 'state' at each interface, and the zone the 
    information comes from depends on the sign of the velocity.
    ...
    my_data : CellCenterData2d object
        ...
        we are advecting.
    u_adv, v_adv: ndarray
        The edge-centered advective velocities
