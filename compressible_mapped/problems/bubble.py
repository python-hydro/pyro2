...
def init_data(my_data, rp):
    ...
    for j in range(myg.jlo, myg.jhi + 1):
        ...
        if j == myg.jlo:
            ...
            p[:, j] = p[:, j - 1] + 0.5 * myg.dy * \
                (dens[:, j] + dens[:, j - 1]) * grav
    ...
    eint = (ener[idx] - 0.5 * (xmom[idx]**2 -
                               ymom[idx]**2) / dens[idx]) / dens[idx]
    ...
    ener[idx] = dens[idx] * eint + 0.5 * \
        (xmom[idx]**2 + ymom[idx]**2) / dens[idx]
